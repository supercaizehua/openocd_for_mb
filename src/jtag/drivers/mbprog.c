 ...
 * mbprog_jtag_nums command or the relevant mbprog_XXX_num commang.
 * The srst and trst gpios are set via the mbprog_srst_num and
 * mbprog_trst_num respectively. GPIO numbering follows the kernel
...
static void mbprog_swdio_drive(bool is_output)
{
	char buf[40];
	int ret;
...
static int mbprog_swdio_read(void)
{
	char buf[1];
...
static void mbprog_swdio_write(int swclk, int swdio)
{
	const char one[] = "1";
	const char zero[] = "0";
 ...
 */
static void mbprog_write(int tck, int tms, int tdi)
{
	if (swd_mode) {
		mbprog_swdio_write(tck, tdi);
		return;
	}
 ...
 */
static void mbprog_reset(int trst, int srst)
{
	LOG_DEBUG("mbprog_reset");
	const char one[] = "1";
	const char zero[] = "0";
	size_t bytes_written;
...
COMMAND_HANDLER(mbprog_handle_jtag_gpionums)
{
	if (CMD_ARGC == 4) {
	...
	command_print(CMD_CTX,
			"Mbprog nums: tck = %d, tms = %d, tdi = %d, tdo = %d",
			tck_gpio, tms_gpio, tdi_gpio, tdo_gpio);

	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_jtag_gpionum_tck)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], tck_gpio);

	command_print(CMD_CTX, "Mbprog num: tck = %d", tck_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_jtag_gpionum_tms)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], tms_gpio);

	command_print(CMD_CTX, "Mbprog num: tms = %d", tms_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_jtag_gpionum_tdo)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], tdo_gpio);

	command_print(CMD_CTX, "Mbprog num: tdo = %d", tdo_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_jtag_gpionum_tdi)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], tdi_gpio);

	command_print(CMD_CTX, "Mbprog num: tdi = %d", tdi_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_jtag_gpionum_srst)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], srst_gpio);

	command_print(CMD_CTX, "Mbprog num: srst = %d", srst_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_jtag_gpionum_trst)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], trst_gpio);

	command_print(CMD_CTX, "Mbprog num: trst = %d", trst_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_swd_gpionums)
{
	if (CMD_ARGC == 2) {
	...
	command_print(CMD_CTX,
			"Mbprog nums: swclk = %d, swdio = %d",
			swclk_gpio, swdio_gpio);

	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_swd_gpionum_swclk)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], swclk_gpio);

	command_print(CMD_CTX, "Mbprog num: swclk = %d", swclk_gpio);
	return ERROR_OK;
}

COMMAND_HANDLER(mbprog_handle_swd_gpionum_swdio)
{
	if (CMD_ARGC == 1)
		COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], swdio_gpio);

	command_print(CMD_CTX, "Mbprog num: swdio = %d", swdio_gpio);
	return ERROR_OK;
}

static const struct command_registration mbprog_command_handlers[] = {
	{
		.name = "mbprog_jtag_nums",
		.handler = &mbprog_handle_jtag_gpionums,
		.mode = COMMAND_CONFIG,
	...
	{
		.name = "mbprog_tck_num",
		.handler = &mbprog_handle_jtag_gpionum_tck,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for tck.",
	},
	{
		.name = "mbprog_tms_num",
		.handler = &mbprog_handle_jtag_gpionum_tms,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for tms.",
	},
	{
		.name = "mbprog_tdo_num",
		.handler = &mbprog_handle_jtag_gpionum_tdo,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for tdo.",
	},
	{
		.name = "mbprog_tdi_num",
		.handler = &mbprog_handle_jtag_gpionum_tdi,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for tdi.",
	},
	{
		.name = "mbprog_srst_num",
		.handler = &mbprog_handle_jtag_gpionum_srst,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for srst.",
	},
	{
		.name = "mbprog_trst_num",
		.handler = &mbprog_handle_jtag_gpionum_trst,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for trst.",
	},
	{
		.name = "mbprog_swd_nums",
		.handler = &mbprog_handle_swd_gpionums,
		.mode = COMMAND_CONFIG,
		.help = "gpio numbers for swclk, swdio. (in that order)",
		.usage = "(swclk swdio)* ",
	},
	{
		.name = "mbprog_swclk_num",
		.handler = &mbprog_handle_swd_gpionum_swclk,
		.mode = COMMAND_CONFIG,
		.help = "gpio number for swclk.",
	},
	{
		.name = "mbprog_swdio_num",
		.handler = &mbprog_handle_swd_gpionum_swdio,
		.mode = COMMAND_CONFIG,
...
static int mbprog_init(void);
static int mbprog_quit(void);

static const char * const mbprog_transports[] = { "jtag", "swd", NULL };

struct jtag_interface mbprog_interface = {
	.name = "mbprog",
	.supported = DEBUG_CAP_TMS_SEQ,
	.execute_queue = bitbang_execute_queue,
	.transports = mbprog_transports,
	.swd = &bitbang_swd,
	.commands = mbprog_command_handlers,
	.init = mbprog_init,
	.quit = mbprog_quit,
};

static struct bitbang_interface mbprog_bitbang = {
	.read = mbprog_read,
	.write = mbprog_write,
	.reset = mbprog_reset,
	.swdio_read = mbprog_swdio_read,
	.swdio_drive = mbprog_swdio_drive,
	.blink = 0
};
...
static bool mbprog_jtag_mode_possible(void)
{
...
static bool mbprog_swd_mode_possible(void)
{
...
static int mbprog_init(void)
{
	bitbang_interface = &mbprog_bitbang;

	LOG_INFO("Mbprog JTAG/SWD bitbang driver");

	if (mbprog_jtag_mode_possible()) {
		if (mbprog_swd_mode_possible())
	...
	} else if (mbprog_swd_mode_possible()) {
	 ...
	 */
	if (tck_gpio >= 0) {
		tck_fd = setup_mbprog(tck_gpio, 1, 0);
		if (tck_fd < 0)
			goto out_error;
	}

	if (tms_gpio >= 0) {
		tms_fd = setup_mbprog(tms_gpio, 1, 1);
		if (tms_fd < 0)
			goto out_error;
	}

	if (tdi_gpio >= 0) {
		tdi_fd = setup_mbprog(tdi_gpio, 1, 0);
		if (tdi_fd < 0)
			goto out_error;
	}

	if (tdo_gpio >= 0) {
		tdo_fd = setup_mbprog(tdo_gpio, 0, 0);
		if (tdo_fd < 0)
			goto out_error;
	}
	...
	/* assume active low*/
	if (trst_gpio >= 0) {
		trst_fd = setup_mbprog(trst_gpio, 1, 1);
		if (trst_fd < 0)
			goto out_error;
	}
	...
	/* assume active low*/
	if (srst_gpio >= 0) {
		srst_fd = setup_mbprog(srst_gpio, 1, 1);
		if (srst_fd < 0)
			goto out_error;
	}

	if (swclk_gpio >= 0) {
		swclk_fd = setup_mbprog(swclk_gpio, 1, 0);
		if (swclk_fd < 0)
			goto out_error;
	}

	if (swdio_gpio >= 0) {
		swdio_fd = setup_mbprog(swdio_gpio, 1, 0);
		if (swdio_fd < 0)
			goto out_error;
	}
